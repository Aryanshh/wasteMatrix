  ...
  useEffect(() => {
    const syncRoute = () => {
      ...
      if (validTabs.includes(hash)) {
        setView('app');
        setActiveTab(hash);
        document.title = `WasteMatrix | ${hash.charAt(0).toUpperCase() + hash.slice(1)}`;
      } else if (hash === 'landing' || hash === '') {
        setView('landing');
        document.title = 'WasteMatrix | Industrial Symbiosis';
      }
    };
  ...
  const LandingHeader = () => (
      <header className="header-fixed flex items-center justify-between px-10">
        ...
        <nav className="hidden md:flex gap-10">
          <a href="#features" className="text-xs font-black uppercase tracking-[0.2em] hover:text-primary transition-colors">Intelligence</a>
          <a href="#network" className="text-xs font-black uppercase tracking-[0.2em] hover:text-primary transition-colors">The Network</a>
          <a href="#pricing" className="text-xs font-black uppercase tracking-[0.2em] hover:text-primary transition-colors">Access</a>
        </nav>

        <div className="flex items-center gap-6">
          <a href="#" className="text-xs font-black uppercase tracking-[0.2em] hidden sm:block hover:underline">Login</a>
          <button onClick={() => setView('app')} className="btn-pill px-8">Enter Platform</button>
        </div>
      </header>
  )
  ...
  const Hero = () => (
    <section className="py-40 px-6 max-w-7xl mx-auto flex flex-col items-center text-center animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-4 w-full max-w-2xl mb-24">
        ...
        <button onClick={() => setView('app')} className="btn-brutal flex items-center justify-center gap-2">
  ...
  const ProblemSolution = () => (
    <section className="flex flex-col md:flex-row border-y-4 border-charcoal relative">
      ...
      <div className="flex-1 bg-charcoal text-white p-12 md:p-24 relative overflow-hidden group">
        ...
        <div className="absolute bottom-[-10%] right-[-5%] font-['Anton'] text-[15vw] text-white opacity-5 select-none pointer-events-none uppercase">VOID</div>
      </div>
      ...
      <div className="flex-1 bg-dark-gray p-12 md:p-24 relative overflow-hidden group border-t-4 md:border-t-0 md:border-l-4 border-primary">
        ...
        <ul className="space-y-8 relative z-10 text-white">
          {[
            'AI-Powered chemical matching', 
            'Real-time logistics routing', 
            'Verified industrial profiles', 
        ...
        <div className="absolute bottom-[-10%] right-[-5%] font-['Anton'] text-[15vw] text-primary opacity-5 select-none pointer-events-none uppercase">FLUX</div>
      </div>
    </section>
  )
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-300">
        ...
        <div className="md:col-span-2 bg-slate-50 border border-charcoal-10 p-10 rounded-2xl flex flex-col justify-between hover:border-primary transition-all duration-300">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { val: '1.2M', label: 'Tonnes Matched' },
              { val: 'Nordic', label: 'Region Hub' },
              { val: '92%', label: 'Avg. Compatibility' }
            ].map(stat => (
  ...
  const FinalCTA = () => (
    <section className="bg-[var(--primary)] py-32 px-6 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <button onClick={() => setView('app')} className="btn-brutal bg-[var(--charcoal)] text-white border-white px-12">Enter The Platform</button>
      </div>
    </section>
  )
